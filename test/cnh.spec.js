...
describe("Parser helper should handle translations (cnh)", () => {
	let p = {}
	beforeEach(() => {
		p = new bcv_parser(lang);
		p.set_options({ book_alone_strategy: "ignore", book_sequence_strategy: "ignore", osis_compaction_strategy: "bc", captive_end_digits_strategy: "delete", testaments: "ona" });
	});
	it("should handle translations (cnh)", () => {
		expect(p.parse("Lev 1 (HCL06)").osis_and_translations()).toEqual([["Lev.1","HCL06"]]);
		expect(p.parse("Lev 1 (HAKABSI)").osis_and_translations()).toEqual([["Lev.1","HAKABSI"]]);
	});
});
describe("Parser helper should handle book ranges (cnh)", () => {
	let p = {}
	beforeEach(() => {
		p = new bcv_parser(lang);
		p.set_options({ book_alone_strategy: "ignore", book_sequence_strategy: "ignore", osis_compaction_strategy: "bc", captive_end_digits_strategy: "delete", testaments: "ona" });
	});
	it("should handle book ranges (cnh)", () => {
