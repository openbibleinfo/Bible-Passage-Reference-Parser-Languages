...
describe("Parser helper should handle translations (bgr)", () => {
	let p = {}
	beforeEach(() => {
		p = new bcv_parser(lang);
		p.set_options({ book_alone_strategy: "ignore", book_sequence_strategy: "ignore", osis_compaction_strategy: "bc", captive_end_digits_strategy: "delete", testaments: "ona" });
	});
	it("should handle translations (bgr)", () => {
		expect(p.parse("Lev 1 (BCLBV)").osis_and_translations()).toEqual([["Lev.1","BCLBV"]]);
	});
});
describe("Parser helper should handle book ranges (bgr)", () => {
	let p = {}
	beforeEach(() => {
		p = new bcv_parser(lang);
		p.set_options({ book_alone_strategy: "ignore", book_sequence_strategy: "ignore", osis_compaction_strategy: "bc", captive_end_digits_strategy: "delete", testaments: "ona" });
	});
	it("should handle book ranges (bgr)", () => {
