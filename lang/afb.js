  ...
  constructor() {
    ...
    this.all_books = [
          ...
          },
          {
            osis: ["Matt"],
            testament: "n",
            regexp: /(?:^|(?<=[^\p{L}]))((?:ال[إا]نجيل\s*زي\s*ما\s*كتبه\s*م(?:َت(?:َ?ّ|َ?)ى|ت(?:َ?ّ|َ?)ى)|م(?:َت(?:َ?ّ|َ?)ى|ت(?:َ?ّ|َ?)ى)))(?:(?=[\d\s\.?:,;\x1e\x1f&\(\)（）\[\]\/"’'\*=~\-–—])|$)/giu
          },
          {
            osis: ["Mark"],
            testament: "n",
            regexp: /(?:^|(?<=[^\p{L}]))((?:[إا]نجيل\s*)?مرقس)(?:(?=[\d\s\.?:,;\x1e\x1f&\(\)（）\[\]\/"’'\*=~\-–—])|$)/giu
          },
          {
            osis: ["Acts"],
            testament: "n",
            regexp: /(?:^|(?<=[^\p{L}]))([أا]ع(?:مال)?)(?:(?=[\d\s\.?:,;\x1e\x1f&\(\)（）\[\]\/"’'\*=~\-–—])|$)/giu
          }
        ];
  }
};
