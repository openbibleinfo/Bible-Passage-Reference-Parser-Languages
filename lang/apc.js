  ...
  constructor() {
    ...
    this.all_books = [
          ...
          },
          {
            ...
            regexp: /(?:^|(?<=[^\p{L}]))(م(?:قتطفات\s*من\s*الزبور|ن\s*الزبور\.?))(?:(?=[\d\s\.?:,;\x1e\x1f&\(\)（）\[\]\/"’'\*=~\-–—])|$)/giu
          },
          {
            osis: ["Jonah"],
            testament: "o",
            regexp: /(?:^|(?<=[^\p{L}]))(قص(?:ّة\s*النبي\s*يونس\.?|ة\s*النبي\s*يونس\.?))(?:(?=[\d\s\.?:,;\x1e\x1f&\(\)（）\[\]\/"’'\*=~\-–—])|$)/giu
          }
        ];
  }
};
